# Meridian v8.1 — Deployment Guide
...
This guide replaces it completely with Meridian v8.1 (the AI margin dashboard).
...
unzip meridianv8.1_final.zip -d ~/projects/meridianv8.1
cd ~/projects/meridianv8.1
...
# Start locally
cd backend
uvicorn indexv8.1:app --reload --port 8000

# Visit: http://localhost:8000/app
# You should see the Meridian v8.1 margin dashboard
```
...
git init
git add .
git commit -m "meridian v8.1"
git remote add origin https://github.com/YOUR_USERNAME/meridianv8.1.git
...
9. Start command (already in `railway.json`): `cd backend && uvicorn indexv8.1:app --host 0.0.0.0 --port $PORT`
...
Vercel will:
- Serve `landing/indexv8.1.html` at `/` (the new landing page)
- Serve `frontend/indexv8.1.html` at `/app` (the dashboard)
...
# Should return: {"status":"ok","version":"8.1.0",...}

# 3. Dashboard
# Visit: https://meridianvisual.io/app
# Should show: Meridian v8.1 — AI Margin Intelligence (with Clerk sign-in)
```
...
│   ├── indexv8.1.py  ← Entry point
│   └── *.py
├── frontend/
│   └── indexv8.1.html  ← Dashboard — served at /app
├── landing/
│   └── indexv8.1.html  ← Landing page — served at /
...
Once you deploy v8.1 via Vercel and point meridianvisual.io at the new Vercel deployment,
the old product disappears and v8.1 takes over.

The old product is completely gone from your codebase — every file in this zip is v8.1 only.
