...
FIXES vs v8.3.4:
  - All backend imports updated to v8_3_5 modules
  - serve_root() / serve_app() now find index.html regardless of versioned filename
  - analyticsv8_3_4 swapped to analyticsv8_3_5 (pulls correct signal field shape)
  - email_servicev8_3_4 swapped to v8_3_5 (password reset URL fixed)
"""
import sys, os
...
from databasev8_3_5 import get_db, create_tables
from modelsv8_3_5 import (
...
import analyticsv8_3_5 as analytics
from proxyv8_3_5 import evaluate_routing_rules, check_wallet, record_transaction, check_and_fire_alerts
from encryptionv8_3_5 import encrypt, decrypt
import email_servicev8_3_5 as email_service

# ── APP ───────────────────────────────────────────────────────────────────

limiter = Limiter(key_func=get_remote_address)
app = FastAPI(title="Meridian API", version="8.3.5")
...
def _seed_pricing():
    from databasev8_3_5 import SessionLocal
    from modelsv8_3_5 import ModelPricing
    db = SessionLocal()
    try:
...
def list_budgets(workspace_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    from analyticsv8_3_5 import period_start
    from sqlalchemy import func
    from modelsv8_3_5 import Transaction, Budget
...
@app.get("/health")
def health():
    return {"status": "ok", "version": "8.3.5", "timestamp": datetime.utcnow().isoformat()}
