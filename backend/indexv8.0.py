...
from databasev8_0 import get_db, create_tables
from modelsv8_0 import (
...
import analyticsv8_0 as analytics
from proxyv8_0 import evaluate_routing_rules, check_wallet, record_transaction, check_and_fire_alerts
from encryptionv8_0 import encrypt, decrypt
import email_servicev8_0 as email_service

# ── APP ───────────────────────────────────────────────────────────────────

limiter = Limiter(key_func=get_remote_address)
app = FastAPI(title="Meridian API", version="8.0.0")
...
def _seed_pricing():
    from databasev8_0 import SessionLocal
    from modelsv8_0 import ModelPricing
    db = SessionLocal()
    try:
...
def list_budgets(workspace_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    from analyticsv8_0 import period_start
    from sqlalchemy import func
    from modelsv8_0 import Transaction, Budget
...
def simulate_pricing(workspace_id: str, body: dict,
                     user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    get_workspace(workspace_id, user, db)
    from modelsv8_0 import Transaction
    from analyticsv8_0 import period_start
...
@app.get("/health")
def health():
    return {"status": "ok", "version": "8.0.0", "timestamp": datetime.utcnow().isoformat()}
