...
from databasev8_1 import get_db, create_tables
from modelsv8_1 import (
...
import analyticsv8_1 as analytics
from proxyv8_1 import evaluate_routing_rules, check_wallet, record_transaction, check_and_fire_alerts
from encryptionv8_1 import encrypt, decrypt
import email_servicev8_1 as email_service

# ── APP ───────────────────────────────────────────────────────────────────

limiter = Limiter(key_func=get_remote_address)
app = FastAPI(title="Meridian API", version="8.1.0")
...
def _seed_pricing():
    from databasev8_1 import SessionLocal
    from modelsv8_1 import ModelPricing
    db = SessionLocal()
    try:
...
def list_budgets(workspace_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    from analyticsv8_1 import period_start
    from sqlalchemy import func
    from modelsv8_1 import Transaction, Budget
...
@app.get("/health")
def health():
    return {"status": "ok", "version": "8.1.0", "timestamp": datetime.utcnow().isoformat()}
