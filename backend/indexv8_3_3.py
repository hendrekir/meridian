...
from databasev8_3_3 import get_db, create_tables
from modelsv8_3_3 import (
...
import analyticsv8_3_3 as analytics
from proxyv8_3_3 import evaluate_routing_rules, check_wallet, record_transaction, check_and_fire_alerts
from encryptionv8_3_3 import encrypt, decrypt
import email_servicev8_3_3 as email_service

# ── APP ───────────────────────────────────────────────────────────────────

limiter = Limiter(key_func=get_remote_address)
app = FastAPI(title="Meridian API", version="8.3.3")
...
def _seed_pricing():
    from databasev8_3_3 import SessionLocal
    from modelsv8_3_3 import ModelPricing
    db = SessionLocal()
    try:
...
def list_budgets(workspace_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    from analyticsv8_3_3 import period_start
    from sqlalchemy import func
    from modelsv8_3_3 import Transaction, Budget
...
def simulate_pricing(workspace_id: str, body: dict,
                     user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    get_workspace(workspace_id, user, db)
    from modelsv8_3_3 import Transaction
    from analyticsv8_3_3 import period_start
...
@app.get("/health")
def health():
    return {"status": "ok", "version": "8.3.3", "timestamp": datetime.utcnow().isoformat()}
