"""
Meridian v8.3.5 — email_service.py
Resend-backed transactional emails: welcome, verify, password reset, alerts.

FIX vs v8.3.4:
  - send_password_reset: link was pointing to /reset-password which had no
    route on the server. Fixed to /app so the frontend JS intercepts it.
"""
import os
import hmac
import hashlib
...
FROM_ADDRESS    = os.getenv("EMAIL_FROM", "Meridian <noreply@meridianvisual.io>")
APP_URL         = os.getenv("APP_URL", "https://meridianvisual.io")
...
def send_password_reset(to: str, user_id: str) -> bool:
    ...
    # FIX: was /reset-password (no route). Frontend JS intercepts token param on /app.
    link  = f"{APP_URL}/app?token={token}&user_id={user_id}"
    html  = f"""
    <div style="font-family:system-ui,sans-serif;max-width:520px;margin:40px auto;color:#1a1a1a">
