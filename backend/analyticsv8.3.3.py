...
def get_signal(db: Session, workspace_id: str) -> dict:
    from modelsv8_3_3 import Transaction, Customer
...
def get_feature_margins(db: Session, workspace_id: str) -> list:
    from modelsv8_3_3 import Feature, Transaction
...
def get_customer_margins(
    ...
    from modelsv8_3_3 import Customer, Transaction
...
# Current pricing for what-if calculations — same as seed data in indexv8.3.3.py
_MODEL_PRICING = {
...
def get_model_comparison(db: Session, workspace_id: str, days: int = 30) -> dict:
    ...
    Returns current spend + ranked alternatives with savings.
    """
    from models_v6_2 import Transaction
    ...
    # For each alternative model, calculate what total spend would be
    # using the actual token volumes from all current usage
    ...
    for model_id, pricing in _MODEL_PRICING.items():
        ...
        # Check if this is already the primary model being used
    ...
    # Sort: current models first, then by saving descending
