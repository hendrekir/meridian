...
from databasev8_3_2 import get_db, create_tables
from modelsv8_3_2 import (
...
import analyticsv8_3_2 as analytics
from proxyv8_3_2 import evaluate_routing_rules, check_wallet, record_transaction, check_and_fire_alerts
from encryptionv8_3_2 import encrypt, decrypt
import email_servicev8_3_2 as email_service

# ── APP ───────────────────────────────────────────────────────────────────

limiter = Limiter(key_func=get_remote_address)
app = FastAPI(title="Meridian API", version="8.3.2")
...
def _seed_pricing():
    from databasev8_3_2 import SessionLocal
    from modelsv8_3_2 import ModelPricing
    db = SessionLocal()
    try:
...
def list_customers(workspace_id: str, status: Optional[str] = None, limit: int = 50, offset: int = 0,
    ...
    ws = get_workspace(workspace_id, user, db)
    # Soft gate — return limited preview for free plan instead of hard 402
...
def list_budgets(workspace_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    from analyticsv8_3_2 import period_start
    from sqlalchemy import func
    from modelsv8_3_2 import Transaction, Budget
...
def list_rules(workspace_id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not PLAN_LIMITS.get(ws.plan or "free", PLAN_LIMITS["free"]).get("routing_rules", False):
        return []   # soft gate — empty list, frontend shows upgrade prompt
...
# Price IDs — set these in Railway env vars once you create products in Stripe dashboard
# STRIPE_PRICE_PRO   = price_xxx   (e.g. $49/mo)
# STRIPE_PRICE_SCALE = price_xxx   (e.g. $199/mo)
...
def simulate_pricing(workspace_id: str, body: dict,
                     user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    get_workspace(workspace_id, user, db)
    from modelsv8_3_2 import Transaction
    from analyticsv8_3_2 import period_start
...
@app.get("/health")
def health():
    return {"status": "ok", "version": "8.3.2", "timestamp": datetime.utcnow().isoformat()}
